...
RSpec.describe 'produce illustration command' do
  ...
  def setup_book_structure
    ...
    File.write('data/world_metadata.yml', "book:\n  current_chapter: 1\n")
  ...
  context 'without world structure' do
    it 'fails when not in a world directory' do
      _, stderr, status = Open3.capture3('ruby', cli_path, 'illustration', '--chapter', '1', '--content', '9:11')
      expect(status).not_to be_success
      expect(stderr).to include('world directory')
    end
  end

  context 'with world structure' do
    before { setup_book_structure }

    it 'validates content range format' do
      stdout, stderr, status = Open3.capture3('ruby', cli_path, 'illustration', '--chapter', '1', '--content', 'invalid')
      expect(status).not_to be_success
    ...
    it 'validates line range is within file bounds' do
      stdout, stderr, status = Open3.capture3('ruby', cli_path, 'illustration', '--chapter', '1', '--content', '1:1000')
      expect(status).not_to be_success
      combined = stdout + stderr
    ...
    it 'validates start line is before end line' do
      stdout, stderr, status = Open3.capture3('ruby', cli_path, 'illustration', '--chapter', '1', '--content', '15:10')
      expect(status).not_to be_success
      combined = stdout + stderr
      expect(combined).to include('Invalid line range')
    end
    ...
    it 'fails when chapter file does not exist' do
      stdout, stderr, status = Open3.capture3('ruby', cli_path, 'illustration', '--chapter', '99', '--content', '1:5')
      expect(status).not_to be_success
      combined = stdout + stderr

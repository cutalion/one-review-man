...
RSpec.describe Eidos::Producers::InstagramComicProducer do
  ...
  before do
    ...
    Eidos::Producer.reset_registry!
    # Re-register after reset
    Eidos::Producer.register(:instagram_comic, described_class)
  ...
  describe '#produce' do
    ...
    context 'with MOCK_AI=true (US1: core generation)' do
      ...
      it 'includes character appearance in image prompts' do
        ...
        allow(Eidos::EnvUtils).to receive(:mock_ai_enabled?).and_call_original
        allow(Eidos::EnvUtils).to receive(:mock_ai_enabled?).and_return(true)
    ...
    context 'US1-007: text control in image prompts' do
      ...
      it 'omits text control when text_elements is nil (LLM did not provide)' do
        ...
        allow_any_instance_of(Eidos::PanelDescriptionGenerator).to receive(:generate).and_return(mock_panels)
  ...
  describe 'Producer Registry (SC-003)' do
    it 'can be found via Producer.find' do
      expect(Eidos::Producer.find(:instagram_comic)).to eq(described_class)
    end

    it 'appears in Producer.all' do
      expect(Eidos::Producer.all).to include(instagram_comic: described_class)
    end
  end
end

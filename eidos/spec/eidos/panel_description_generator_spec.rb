...
require 'eidos/panel_description_generator'
require 'eidos/character_appearance'

RSpec.describe Eidos::PanelDescriptionGenerator do
  let(:llm_service) { double('LLMService') }
  let(:generator) { described_class.new(llm_service) }
  let(:characters) do
    {
      'kenji_yamamoto' => Eidos::CharacterAppearance.new(
        'id' => 'kenji_yamamoto',
        'name' => 'Kenji Yamamoto',
        'physical_appearance' => {
          'age' => 28,
  ...
  describe '#generate' do
    context 'with MOCK_AI=true' do
      before { allow(Eidos::EnvUtils).to receive(:mock_ai_enabled?).and_return(true) }
    ...
    context 'with real LLM' do
      before { allow(Eidos::EnvUtils).to receive(:mock_ai_enabled?).and_return(false) }

...
RSpec.describe Eidos::StoryBible do
  ...
  describe '.from_snapshot' do
    let(:snapshot_store) { Eidos::SnapshotStore.new(story_bible_path: story_bible.story_bible_path) }
    ...
    it 'returns a StoryBible reading from snapshot directory' do
      ...
      expect(loaded).to be_a(Eidos::StoryBible)
    end
    ...
    it 'raises FrozenSnapshotError on write attempts' do
      ...
      expect { loaded.save_character('test', { 'name' => 'Test' }) }.to raise_error(Eidos::FrozenSnapshotError)
      expect { loaded.save_location('test', { 'name' => 'Test' }) }.to raise_error(Eidos::FrozenSnapshotError)
      expect { loaded.add_fact('cat', 'id', { 'desc' => 'x' }) }.to raise_error(Eidos::FrozenSnapshotError)
      expect { loaded.add_relationship({ 'character1' => 'a', 'character2' => 'b' }) }.to raise_error(Eidos::FrozenSnapshotError)
      expect { loaded.add_plot_thread({ 'id' => 'x' }) }.to raise_error(Eidos::FrozenSnapshotError)
    end
    ...
    it 'raises SnapshotCorruptError when snapshot data is incomplete' do
      ...
      end.to raise_error(Eidos::SnapshotCorruptError)
    end

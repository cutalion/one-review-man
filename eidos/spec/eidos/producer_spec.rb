...
RSpec.describe Eidos::Producer do
  ...
  describe 'Registry' do
    it 'registers a producer' do
      Eidos::Producer.register(:test, test_producer_class)
      expect(Eidos::Producer.find(:test)).to eq(test_producer_class)
    end
    ...
    it 'returns nil for unregistered producer' do
      expect(Eidos::Producer.find(:nonexistent)).to be_nil
    end

    it 'lists all registered producers' do
      Eidos::Producer.register(:test, test_producer_class)
      all = Eidos::Producer.all
    ...
    it 'resets registry' do
      Eidos::Producer.register(:test, test_producer_class)
      Eidos::Producer.reset_registry!
      expect(Eidos::Producer.all).to be_empty
    end
  end
  ...
  describe 'extensibility (SC-002)' do
    it 'allows adding a new producer with only interface + registration' do
      ...
      Eidos::Producer.register(:dummy, dummy_producer_class)

      # Verify it's discoverable
      expect(Eidos::Producer.find(:dummy)).to eq(dummy_producer_class)

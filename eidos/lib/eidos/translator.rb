...
module Eidos
  # Service class for translating world content to different languages
  class Translator
    include Eidos::Utils
    ...
    def initialize(model_override = nil, llm_service: nil, project_root: Dir.pwd, config: nil)
      ...
      if config
        @llm_service = llm_service || Eidos::LLMService.new(config)
      else
    ...
    def load_book_metadata_abs
      path = File.join(@project_root, 'data', 'world_config.yml')
      path = File.join(@project_root, 'data', 'world_metadata.yml') unless File.exist?(path)

...
module Eidos
  module Storage
    module YamlFile
      # File-based snapshot storage using directory copies.
      # Extracted from the original SnapshotStore.
      class SnapshotStorage
        ...
        attr_reader :snapshots_path

        def initialize(entity_storage:, story_bible_path: nil)
          @entity_storage = entity_storage
          @story_bible_path = story_bible_path || entity_storage.story_bible_path
        ...
        def snapshot_data(name_or_version)
          ...
          dir = snapshot_dir_path(manifest)
          return nil unless Dir.exist?(dir)

          {
            'characters' => load_entities_from_dir(File.join(dir, 'characters')),
            'locations' => load_entities_from_dir(File.join(dir, 'locations')),
            'facts' => (load_yaml_file(File.join(dir, 'facts.yml'))['facts'] || {}),
            'relationships' => (load_yaml_file(File.join(dir, 'relationships.yml'))['relationships'] || []),
            'plot_threads' => (load_yaml_file(File.join(dir, 'plot_threads.yml'))['plot_threads'] || [])
          }
        end

        # Backward compatibility — used by existing SnapshotStore wrapper
        def snapshot_path(name_or_version)
          manifest = get(name_or_version)
          return nil unless manifest

          snapshot_dir_path(manifest)
        end
        ...
        def snapshot_dir_path(manifest)

...
# Common utilities and constants for world project management
module Eidos::Utils
  DATA_DIR = 'data'
  ...
  def load_book_data(lang = nil)
    # Use WorldConfig to load data (handles split config/state transparently)
    unless defined?(Eidos::Config)
    ...
    config = Eidos::WorldConfig.load_from_project(project_root)
  ...
  # Centralised YAML loading – delegates to Eidos::Config so that all YAML
  # access points go through a single helper. Returns an empty Hash for
  # missing files.
  def load_yaml_file(file_path)
    ...
    unless defined?(Eidos::Config)
      core_lib = File.expand_path('../../..', __dir__)
      $LOAD_PATH.unshift(core_lib) unless $LOAD_PATH.include?(core_lib)
      require 'eidos/config'
    end

    Eidos::Config.load_yaml(file_path)
  end

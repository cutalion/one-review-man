...
RSpec.describe 'book generate command' do
  ...
  it 'supports --auto flag in book directory' do
    Dir.mktmpdir do |tmpdir|
      Dir.chdir(tmpdir) do
        ...
        stdout, stderr, status = Open3.capture3('ruby', cli_path, 'generate', 'chapter', '--auto')
        expect(status).to be_success
        expect(stderr).to be_empty
        expect(stdout).to include('No chapters found')
      end
    end
  end
  ...
  it 'accepts --model option in book directory' do
    Dir.mktmpdir do |tmpdir|
      Dir.chdir(tmpdir) do
        FileUtils.mkdir_p('_chapters')
        stdout, stderr, status = Open3.capture3('ruby', cli_path, 'generate', 'chapter', '1', '--model', 'gpt-4o', '--auto')
        expect(status).to be_success

...
RSpec.describe BookCore::BookConfig do
  ...
  describe '#initialize' do
    ...
    it 'validates structure on initialization' do
      expect { described_class.new('invalid', {}) }.to raise_error(BookCore::BookConfig::ValidationError, 'Config data must be a Hash')
      expect { described_class.new({}, 'invalid') }.to raise_error(BookCore::BookConfig::ValidationError, 'State data must be a Hash')
    end
  end
  ...
  describe '.load_from_project' do
    ...
    let(:config_path) { File.join(temp_dir, 'data', 'book_config.yml') }
    let(:state_path) { File.join(temp_dir, 'data', 'book_state.yml') }
    let(:legacy_path) { File.join(temp_dir, 'data', 'book_metadata.yml') }
  ...
  describe 'persistence' do
    ...
    describe '#save!' do
      ...
      it 'creates directory if it does not exist' do
        ...
        config_path = File.join(temp_dir, 'nested', 'book_config.yml')
        state_path = File.join(temp_dir, 'nested', 'book_state.yml')

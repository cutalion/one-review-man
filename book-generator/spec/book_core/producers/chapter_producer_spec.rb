...
RSpec.describe BookCore::Producers::ChapterProducer do
  ...
  before do
    ...
    # Create minimal book_metadata.yml
    File.write(File.join(tmp_dir, 'data', 'book_metadata.yml'), {
      'title' => 'Test Book',
      'current_chapter' => 0,
      'total_chapters' => 10
    }.to_yaml)
  ...
  describe '#produce' do
    ...
    it 'returns a ProducerResult', :aggregate_failures do
      ...
      generator = instance_double(BookCore::ChapterGenerator)
      allow(BookCore::ChapterGenerator).to receive(:new).and_return(generator)
      ...
      expect(result).to be_a(BookCore::ProducerResult)
    ...
    it 'passes snapshot to ChapterGenerator' do
      ...
      expect(BookCore::ChapterGenerator).to have_received(:new).with(
  ...
  describe 'registration' do
    it 'is registered in the Producer registry' do
      ...
      BookCore::Producer.register(:chapter, described_class)
      expect(BookCore::Producer.find(:chapter)).to eq(described_class)
    end
  end
end

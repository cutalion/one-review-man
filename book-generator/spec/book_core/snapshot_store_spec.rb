...
RSpec.describe BookCore::SnapshotStore do
  ...
  describe 'name validation' do
    ...
    it 'rejects names with invalid characters' do
      %w[UPPERCASE has_underscore has.dot has\ space].each do |name|
        expect { store.create(name: name) }.to raise_error(BookCore::InvalidSnapshotNameError)
      end
    end

    it 'rejects names starting with a hyphen' do
      expect { store.create(name: '-bad') }.to raise_error(BookCore::InvalidSnapshotNameError)
    end

    it 'rejects names exceeding 64 characters' do
      long_name = 'a' * 65
      expect { store.create(name: long_name) }.to raise_error(BookCore::InvalidSnapshotNameError)
    end
  ...
  describe '#create' do
    ...
    it 'rejects duplicate snapshot names' do
      ...
      expect { store.create(name: 'initial') }.to raise_error(
        BookCore::DuplicateSnapshotError, /already exists/
      )
    end

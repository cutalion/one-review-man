...
RSpec.describe 'book snapshot CLI' do
  let(:cli_path) { File.expand_path('../bin/book', __dir__) }
  let(:test_dir) { Dir.mktmpdir('snapshot_cli_test') }

  before do
    # Initialize a minimal book project
    stdin_data = "Test Book\nTest Author\nA test book\nen\nen\n"
    Open3.capture3('ruby', cli_path, 'init', '--book-dir', test_dir, '--quick', stdin_data: stdin_data)
  end
  ...
  describe 'snapshot create' do
    it 'creates a snapshot and shows confirmation' do
      stdout, stderr, status = Open3.capture3('ruby', cli_path, 'snapshot', 'create', 'initial', '-b', test_dir)
    ...
    it 'rejects duplicate names' do
      Open3.capture3('ruby', cli_path, 'snapshot', 'create', 'initial', '-b', test_dir)
      _stdout, stderr, status = Open3.capture3('ruby', cli_path, 'snapshot', 'create', 'initial', '-b', test_dir)
    ...
    it 'rejects invalid names' do
      _stdout, stderr, status = Open3.capture3('ruby', cli_path, 'snapshot', 'create', 'INVALID', '-b', test_dir)
  ...
  describe 'snapshot list' do
    it 'shows "No snapshots found" when empty' do
      stdout, _stderr, status = Open3.capture3('ruby', cli_path, 'snapshot', 'list', '-b', test_dir)
    ...
    it 'lists created snapshots' do
      Open3.capture3('ruby', cli_path, 'snapshot', 'create', 'first', '-b', test_dir)
      Open3.capture3('ruby', cli_path, 'snapshot', 'create', 'second', '-b', test_dir)
  ...
  describe 'snapshot show' do
    it 'shows snapshot details' do
      ...
      stdout, _stderr, status = Open3.capture3('ruby', cli_path, 'snapshot', 'show', 'initial', '-b', test_dir)
    ...
    it 'reports error for non-existent snapshot' do
      _stdout, stderr, status = Open3.capture3('ruby', cli_path, 'snapshot', 'show', 'nonexistent', '-b', test_dir)

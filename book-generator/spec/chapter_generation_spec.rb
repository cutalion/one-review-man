...
RSpec.describe 'book chapter generation' do
  ...
  describe 'canon version recording' do
    let(:project_with_bible) do
      ...
      File.write(File.join(dir, 'data', 'book_config.yml'), <<~YAML)
        generation:
          chapter_length_target: "500-1000 words"
          main_characters: []
          content_rules:
        ...
        localized:
          en:
            ...
            genre: "Comedy"
      YAML
      File.write(File.join(dir, 'data', 'book_state.yml'), "book:\n  current_chapter: 0\n  target_chapters: 10\n")
    ...
    it 'includes canon_version as "unversioned" when no snapshots exist' do
      ...
      config = BookCore::BookConfig.load_from_project(dir)
      configuration = BookCore::Configuration.load(dir, {})
      generator = BookCore::ChapterGenerator.new(
        configuration: configuration,
        project_root: dir,
        book_config: config
  ...
  describe 'integrated chapter generation' do
    it 'calls the ChapterGenerator with correct options' do
      ...
      expect(BookCore::ChapterGenerator).to receive(:new).with(hash_including(configuration: kind_of(Hash), project_root: kind_of(String))).and_call_original

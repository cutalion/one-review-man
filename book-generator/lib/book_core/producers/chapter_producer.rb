  ...
  module Producers
    class ChapterProducer
      ...
      def build_generator(snapshot:, config:, output:)
        ...
        if model
          kwargs[:configuration] = BookCore::Configuration.load(@project_root, { 'llm.model' => model })
        else
          kwargs[:configuration] = BookCore::Configuration.load(@project_root, {})
        end

        BookCore::ChapterGenerator.new(**kwargs)
      end
